...
st.markdown("""
Welcome to the class data competition! 
Submit your predictions to see how you rank against your peers.
The evaluation metric is **Root Mean Squared Error (RMSE)**. Lower is better!
""")

# --- Google Sheets Connection ---
# This uses the credentials and sheet URL from Streamlit's secrets manager
try:
    conn = st.connection("gsheets", type="legacy_gsheets")
except Exception as e:
    st.error("Could not connect to Google Sheets. Please ensure your secrets are configured correctly in the app settings.")
    st.stop()


# --- Helper Functions ---
@st.cache_data(ttl=60) # Cache the leaderboard data for 60 seconds
def fetch_leaderboard():
    """Fetches and sorts the leaderboard from Google Sheets."""
    try:
        ...
        st.error(f"Failed to read from Google Sheets. Is the sheet shared with your service account's client_email? Error: {e}")
...
try:
    ...
    st.error("Solution data not found in secrets. Please configure your secrets in the Streamlit app settings.")
    st.stop()
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("📄 Resources")
    # This assumes sample_submission.csv is in your GitHub repo
    try:
        ...
        st.warning("`sample_submission.csv` not found in repository.")
...
if submit_button:
    if not team_name:
        ...
        try:
            ...
            st.cache_data.clear() # Force a re-fetch of the leaderboard
...
st.markdown("Results are updated in real-time. Lower RMSE is better!")
